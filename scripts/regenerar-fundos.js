...
async function main() {
    ...
    const backgrounds = manifesto.imagens.filter(i => i.tipo === 'background');

    console.log(`Regenerando ${backgrounds.length} fundos com prompts sem texto...\n`);

    for (let i = 0; i < backgrounds.length; i++) {
        const item = backgrounds[i];
        const destPath = path.join(__dirname, '..', item.arquivo);

        // Adiciona instrucoes para nao gerar texto
        ...
        console.log(`[${i + 1}/${backgrounds.length}] REGERANDO: ${item.arquivo}`);
